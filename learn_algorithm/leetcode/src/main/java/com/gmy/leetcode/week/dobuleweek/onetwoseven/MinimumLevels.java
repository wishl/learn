...
public class MinimumLevels {
    ...
    /**
     * 解题参考 https://leetcode.cn/problems/minimum-levels-to-gain-more-points/solutions/2716467/qian-hou-zhui-fen-jie-pythonjavacgo-by-e-t3eu/
     * @param possible
     * @return
     */
    public int minimumLevels(int[] possible) {
        int sum = 0, minLength = Integer.MAX_VALUE, aliceScore = 0;
        for (int result : possible) {
            sum += result == 0 ? -1 : 1;
        }
...
//        while (end < possible.length) {
//            aliceScore += possible[end] == 0 ? -1 : 1;
//            while (aliceScore > sum - aliceScore && start <= end && end - start + 1 < possible.length) {
//                minLength = Math.min(minLength, end - start + 1);
//                aliceScore -= possible[start];
//                start++;
//            }
//            end++;
//        }
    ...
    public static void main(String[] args) {
        MinimumLevels minimumLevels = new MinimumLevels();
        int result = minimumLevels.minimumLevels(new int[]{0,1,0});
